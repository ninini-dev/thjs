...
class EnemySystem{
    ...
    update(delta) {
        ...
        for (let i = 0; i < this.x.length; ) {
            ...
            Renderer.writeVBO([this.x[i],this.y[i],ENEMY_ASPECT_DATA[this.asp[i]],0],i)
            //device.queue.writeBuffer(storageBuffer,i*8, [this.x[i],this.y[i],ENEMY_ASPECT_DATA[this.asp[i]],0]);
